...
public class MoveBehaviour : GenericBehaviour
{
    ...
    public bool jumping; // Boolean to determine whether or not the player started a jump.
   ...
   // float v2 = 1f;
    ...
    void FloatCollider()
    {
        transform.position = Vector3.Lerp(transform.position, new Vector3(transform.position.x, GetFloorCast().y, transform.position.z), Time.deltaTime / 0.1f);
    }
    ...
    {
        // Start a new jump.
        if (jumping && !behaviourManager.GetAnim.GetBool(jumpBool) && behaviourManager.IsGrounded())
        {
        ...
        // Is already jumping?
        else if (jumping && behaviourManager.GetAnim.GetBool(jumpBool))
        {
